...
const getEvents = async () => {
  ...
  const actionsToProcess = result.docs.filter(doc => {
    return doc.status !== "done" && doc.status !== "archived"
  });

  if (actionsToProcess.length === 0) {
    const resultToProcess = await eventsDb.find({
      selector: {
        type: "EVENT",
        status: {
          "$nin": ["error", "done", "archived"]
        }
      },
      limit: 100,
      sort: [
    ...
    const archivedEvents = await eventsDb.find({
      selector: {
        type: "EVENT",
        status: "archived"
      },
    ...
    return [...result.docs, ...resultToProcess.docs, ...archivedEvents.docs];
  }
